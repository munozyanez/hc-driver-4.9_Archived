/* Linux kernel module for HiCO.CAN-MiniPCI CAN-bus cards. Only 3.2 kernels
 * are supported 
 *
...
#define REQUIRED_FW2_VERSION 1488
...

{
    ...
    struct hcan_board *board;
    board=PDE_DATA(file_inode(filp));
	...
	if(board->node[i].disabled) continue;
	if(!first) len+=sprintf(buf+len,", ");
	first=0;
	len+=sprintf(buf+len,"can%d",
		board->node[i].minor);
    }
    len+=sprintf(buf+len,"\n");
...
//           HCANPCI_DRIVER_VERSION,/*"Jan 01 2017","00:00:00");//;*/__DATE__,__TIME__);
...
//	*eof = 1; //used on previuos code
	...
	len+=sprintf(buf+len,
		...
		"requires FW2 version %d or above!!\n\n",REQUIRED_FW2_VERSION);
	return len;
    }
	...
	if(board_cmd(board, CMD_PRINT_EXCEPTION, 0, 0,NULL)){
	    ...
	    char *str = kmalloc(8*1024, GFP_KERNEL);
	    if(!str){
		len+=sprintf(buf+len,"failed to get memory for exception string!!\n");
	    } else {
		memcpy_fromio(str,board->dpm_base,8*1024);
		len+=sprintf(buf+len,"-------------------------------------------------------------\n");
...
#endif
{
	...
	if(reason&INT_CAN3_RX   ) printk(" INT_CAN3_RX");
	if(reason&INT_CAN3_TX   ) printk(" INT_CAN3_TX");
	if(reason&INT_CAN4_RX   ) printk(" INT_CAN4_RX");
	if(reason&INT_CAN4_TX   ) printk(" INT_CAN4_TX");
	printk("\n");
    }
...
static int hcan_pci_probe (struct pci_dev *pdev, const struct pci_device_id *pci_id)
{
    ...
    /* Get EEPROM Revision code. 'Old' MiniPCI cards have revision 0xC (only
     * 8kB DPM window. MiniPCI-4C cards have 0xD (or higher) */
    pci_read_config_byte(board->pdev,8,&board->pci_eeprom_rev);

    /* Set the dpm pointer to point to the control area. Note, that it
     * doesn't really matter here if we have a 16kB window with 8K's of DPM.
     * The 8k is also mirrored in the upper half of the 16k window */
    board->dpm=SET_DPM_PTR(board->dpm_base,pci_resource_len(pdev,2));

    switch(ioread8(&board->dpm->board_status.hw_id)){
	case HW_HICOCAN_MPCI:
	case HW_HICOCAN_PCI104:
	case HW_HICOCAN_UNKNOWN:
	    board->dpm_size=8*1024;
	    break;
	case HW_HICOCAN_MPCI_4C:
	    board->dpm_size=16*1024;
	    break;
	default:
	    board->dpm_size = pci_resource_len(pdev,2);
	    printk(KERN_WARNING "%s: unknown hicocan hardware id %x",
		    __func__,ioread8(&board->dpm->board_status.hw_id));
	    break;
    }

    /* Fix the DPM info structure pointer to it's correct offset */
    board->dpm=SET_DPM_PTR(board->dpm_base,board->dpm_size);
...
//				   board);
	...
	if(fw_update){
	    ...
	    if(ioread16(&node->can_status->can_type)==CAN_TYPE_EMPTY){
		node->disabled=1;
	    }
	}
	 ...
	 * wild pointer */
	if(ioread16(&node->dpm_rxbuf.vars->base) > DPM_MSG_AREA_SIZE(board->dpm_size) || 
	    ioread16(&node->dpm_txbuf.vars->base) > DPM_MSG_AREA_SIZE(board->dpm_size) ||
	    ioread16(&node->dpm_rxbuf.vars->size) > (DPM_MSG_AREA_SIZE(board->dpm_size)/sizeof(BUF_UNIT)) ||
	    ioread16(&node->dpm_txbuf.vars->size) > (DPM_MSG_AREA_SIZE(board->dpm_size)/sizeof(BUF_UNIT))){
	    if(!fw_update){
	...
	switch(i){
		...
		break;
	    case 2:
		node->tx_int=INT_CAN3_TX; 
		node->rx_int=INT_CAN3_RX; 
		break;
	    case 3:
		node->tx_int=INT_CAN4_TX; 
		node->rx_int=INT_CAN4_RX; 
		break;
	}
...
//				   node);
    ...
    printk(KERN_INFO "%s: board %s initialized.\n",
	       __FUNCTION__, pci_name(pdev));
