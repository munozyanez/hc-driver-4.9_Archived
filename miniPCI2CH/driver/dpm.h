...
/* On the HiCO Firmware the pointers are real pointers in the processor
 * address space. On the host we use only the two lsb as offset. the __res*
 * fields should never be touched */
...
#define SIZE_OF_DPM (1024*8)
#define DPM_MSG_AREA_SIZE (SIZE_OF_DPM - sizeof(struct dpm))

/* Size of data blocks when updating firmware via dpm */
#define FW_UPDATE_BLOCK_SIZE 0x1000


/* Macro to set a pointer to the control area of the dpm. Takes the dpm base
 * address as argument */
#define SET_DPM_PTR(dpm_base) ((struct dpm *)((uint8_t *)dpm_base + SIZE_OF_DPM - sizeof(struct dpm)))
...
#define CMD_INIT_LATTE 0x84
#define CMD_LATTE 0x85
...
#define INT_CAN1_ERROR  (1<<2)
#define INT_CAN2_RX	(1<<4)
#define INT_CAN2_TX	(1<<5)
#define INT_CAN2_ERROR	(1<<6)
...
#if !defined(FW1) && !defined(FW2) && !defined(__KERNEL__)
#error -- not include dpm.h to your application - use hico_api.h instead!
#endif
